...
def get_league_name_from_league_id(league_id):
    url = f"https://api.sleeper.app/v1/league/{league_id}"
    response = requests.get(url)
    
    if response.status_code == 200:
        league_data = response.json()
        return league_data.get('name', 'Unbekannte Liga')  # Rückgabe des Namens oder 'Unbekannte Liga', wenn Name nicht vorhanden
    else:
...
def prepare_data(selected_leagues=None, search_query=None):
    ...
    for league_id in league_ids:
        ...
        league_name = get_league_name_from_league_id(league_id)  # Liga-Name abrufen
        users = get_users_from_league(league_id)
        
        # Füge einen Knoten für jede Liga hinzu, mit dem Namen der Liga
        nodes.append({"data": {"id": f"league_{league_id}", "label": "LEAGUE", "name": league_name}})
...
league_names = [get_league_name_from_league_id(league_id) for league_id in league_ids]
